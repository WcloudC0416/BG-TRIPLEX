...
from get_patch_csv import edge_per_patch_out_dir
...
def collate_fn(batch: tuple):
    ...
    edge_patch = torch.stack([item[1] for item in batch])
    nuc_patch = torch.stack([item[2] for item in batch])
    n_edge_patch = torch.stack([item[3] for item in batch])
    n_nuc_patch = torch.stack([item[4] for item in batch])
    exp = torch.stack([item[5] for item in batch])
    pid = torch.stack([item[6] for item in batch])
    sid = torch.stack([item[7] for item in batch])
    wsi = [item[8] for item in batch]
    position = [item[9] for item in batch]
    neighbors = torch.stack([item[10] for item in batch])
    mask = torch.stack([item[11] for item in batch])
    
    return patch, edge_patch, nuc_patch, n_edge_patch, n_nuc_patch, exp, pid, sid, wsi, position, neighbors, mask
