...
from models.module import GlobalEncoder, NeighborEncoder, FusionEncoder, NeighborFeatureExtractor, GuidedFeatureEncoder, \
...
class BG_TRIPLEX(pl.LightningModule):
    """Model class for BG_TRIPLEX
    """
    ...
    def __init__(self,
        ...
        self.guided_feature_encoder = GuidedFeatureEncoder(emb_dim, depth1, num_heads1, int(emb_dim * mlp_ratio1),
                                                           dropout1)

        # Neighbor Encoder
        self.neighbor_encoder = NeighborEncoder(emb_dim, depth3, num_heads3, int(emb_dim * mlp_ratio3),
                                                dropout=dropout3, resolution=res_neighbor)
        self.fc_neighbor = nn.Linear(emb_dim, num_genes)

        # Global Encoder
        self.global_encoder = GlobalEncoder(emb_dim, depth2, num_heads2, int(emb_dim * mlp_ratio2), dropout2,
                                            kernel_size)
    ...
    def forward(self, x, x_edge, x_seg, x_n_edge, x_n_seg, x_total, position, neighbor, mask, pid=None, sid=None,
                return_emb=False):
        """Forward pass of BG_TRIPLEX
        ...
        Args:
            ...
            x_edge (torch.Tensor): Edge features of spot image in the patient. (batch_size x 1 x 512)
            x_seg (torch.Tensor): Seg features of spot image in the patient. (batch_size x 1 x 512)
            x_n_edge (torch.Tensor): Edge features of neighbor image in the patient.
            x_n_seg (torch.Tensor): Seg features of neighbor image in the patient.
        ...
        initial_target_token = target_token

        if x_edge.dim() == 2:
            x_edge = x_edge.unsqueeze(0)
        if x_seg.dim() == 2:
            x_seg = x_seg.unsqueeze(0)
        edge_features = self.target_feature_extractor(x_edge).to(x.device)
        seg_features = self.target_feature_extractor(x_seg).to(x.device)

        # neighbor_token = neighbor_token + neighbor_edge_features_adjusted + neighbor_seg_features_adjusted
        target_token = self.guided_feature_encoder(edge_features, seg_features, target_token)

        # print(target_token.shape)
        # print(edge_features_adjusted.shape)
        # print(seg_features_adjusted.shape)

        # target_token = target_token + edge_features_adjusted + seg_features_adjusted
        ...
        initial_neighbor_token = neighbor_token

        if x_n_edge.dim() == 2:
            x_n_edge = x_n_edge.unsqueeze(0)
        if x_n_seg.dim() == 2:
            x_n_seg = x_n_seg.unsqueeze(0)
        neighbor_edge_features_adjusted = self.neighbor_feature_extractor(x_n_edge).to(x.device)
        neighbor_seg_features_adjusted = self.neighbor_feature_extractor(x_n_seg).to(x.device)

        # neighbor_token = neighbor_token + neighbor_edge_features_adjusted + neighbor_seg_features_adjusted
        neighbor_token = self.guided_feature_encoder(neighbor_edge_features_adjusted, neighbor_seg_features_adjusted,
                                                     neighbor_token)
        ...
        out_initial_target = self.fc_target(initial_target_token.mean(1))
        ...
        out_initial_neighbor = self.fc_neighbor(initial_neighbor_token.mean(1))  # B x num_genes
        out_global = self.fc_global(global_token)  # B x num_genes
        return output, out_initial_target, out_target, out_initial_neighbor, out_neighbor, out_global
        ...
        # out_target = self.fc_target(target_token.mean(1))  # B x num_genes
        # out_global = self.fc_global(global_token)  # B x num_genes
        # return output, out_target, out_global

    def training_step(self, batch, batch_idx):
        """Train the model. Transfer knowledge from fusion to each module.

        """
        patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, exp, pid, sid, wsi, position, neighbor, mask = batch
        outputs = self(patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, wsi, position, neighbor,
                       mask, pid, sid)
        ...
        # # Initial Target loss
        # loss += F.mse_loss(outputs[1].view_as(exp), exp) * (1 - self.alpha)  # Supervised loss for Initial Target
        # loss += F.mse_loss(outputs[0], outputs[1]) * self.alpha  # Distillation loss for Initial Target

        # # Target loss
        # loss += F.mse_loss(outputs[2].view_as(exp), exp) * (1 - self.alpha)  # Supervised loss for Target
        # loss += F.mse_loss(outputs[0], outputs[2]) * self.alpha  # Distillation loss for Target

        # # Initial Neighbor loss
        # loss += F.mse_loss(outputs[3].view_as(exp), exp) * (1 - self.alpha)  # Supervised loss for Initial Neighbor
        # loss += F.mse_loss(outputs[0], outputs[3]) * self.alpha  # Distillation loss for Initial Neighbor

        # # Neighbor loss
        # loss += F.mse_loss(outputs[4].view_as(exp), exp) * (1 - self.alpha)  # Supervised loss for Neighbor
        # loss += F.mse_loss(outputs[0], outputs[4]) * self.alpha  # Distillation loss for Neighbor

        # # Global loss
        # loss += F.mse_loss(outputs[5].view_as(exp), exp) * (1 - self.alpha)  # Supervised loss for Global
        # loss += F.mse_loss(outputs[0], outputs[5]) * self.alpha  # Distillation loss for Global
    ...
    def validation_step(self, batch, batch_idx):
        ...
        """
        patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, exp, _, wsi, position, name, neighbor, mask = batch
        patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, exp, neighbor, mask = patch.squeeze(), edge_patch.squeeze(), seg_patch.squeeze(), neighbor_edge_patch.squeeze(), neighbor_seg_patch.squeeze(), exp.squeeze(), neighbor.squeeze(), mask.squeeze()
    ...
    def test_step(self, batch, batch_idx):
        ...
        Returns:
            dict:
                ...
                corr: PCC between pred and label (across genes)
        """
        patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, exp, sid, wsi, position, name, neighbor, mask = batch
        patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, exp, sid, neighbor, mask = patch.squeeze(), edge_patch.squeeze(), seg_patch.squeeze(), neighbor_edge_patch.squeeze(), neighbor_seg_patch.squeeze(), exp.squeeze(), sid.squeeze(), neighbor.squeeze(), mask.squeeze()

        print(name)
        if '10x_breast' in name[0]:
            outputs = self(patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, wsi, position,
                           neighbor.squeeze(), mask.squeeze())
            pred = outputs[0].squeeze()
            ind_match = np.load(f'data/test/{name[0]}/ind_match.npy', allow_pickle=True)
        ...
        outputs = [{"MSE": mse, "MAE": mae, "corr": rr}]
    ...
    def predict_step(self, batch, batch_idx):
        ...
        patches, edge_patches, seg_patches, neighbor_edge_patches, neighbor_seg_patches, sids, wsi, position, neighbors, masks = batch
        patches, edge_patches, seg_patches, neighbor_edge_patches, neighbor_seg_patches, sids, neighbors, masks = patches.squeeze(), edge_patches.squeeze(), seg_patches.squeeze(), neighbor_edge_patches.squeeze(), neighbor_seg_patches.squeeze(), sids.squeeze(), neighbors.squeeze(), masks.squeeze()

        patches = patches.split(512, dim=0)
        edge_patches = edge_patches.split(512, dim=0)
        seg_patches = seg_patches.split(512, dim=0)
        neighbor_edge_patches = neighbor_edge_patches.split(512, dim=0)
        neighbor_seg_patches = neighbor_seg_patches.split(512, dim=0)
        neighbors = neighbors.split(512, dim=0)
        masks = masks.split(512, dim=0)
        sids = sids.split(512, dim=0)

        preds, embs = [], []
        for patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, neighbor, mask, sid in zip(patches,
                                                                                                              edge_patches,
                                                                                                              seg_patches,
                                                                                                              neighbor_edge_patches,
                                                                                                              neighbor_seg_patches,
                                                                                                              neighbors,
                                                                                                              masks,
                                                                                                              sids):
            outputs = self(patch, edge_patch, seg_patch, neighbor_edge_patch, neighbor_seg_patch, wsi, position,
                           neighbor, mask, sid=sid, return_emb=True)
            pred = outputs[0].cpu()
            emb = outputs[1].cpu()
